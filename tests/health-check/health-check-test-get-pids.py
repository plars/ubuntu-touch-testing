...
default_threshold_path = os.path.join(os.path.join(basedir, 'thresholds'), platform.machine())

ignore_procs = [
    'health-check', 'init', 'cat', 'vi', 'emacs', 'getty', 'csh', 'bash', 'sh',
    'powerd', 'thermald', 'mpdecision', 'polkitd', 'whoopsie',
    ...
    for p in psutil.process_iter():
        try:
            if os.getpgid(p.pid) != 0:
                ...
                fname = os.path.join(default_threshold_path, procname + ".threshold")
